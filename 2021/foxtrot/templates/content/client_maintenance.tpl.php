...
$(document).on('change', '#is_reviewed', function(event) {
    if ($(this).is(':checked')) {
        let current_date_time=moment().format('MM/DD/YYYY h:mm:ss A');
        $(this).val(current_date_time)
        $("#current_time").text( current_date_time);
        $("#supervisor_wrap").show();        
    }
    else {
        $(this).val("");
        $("#current_time").text("");
                                                        ...
                                                        <div class="col-md-4">
                                                            <div class="reviewed">
                                                                <div class="check-review">
                                                                    Reviewed
                                                                    <input type="checkbox" name="is_reviewed" value="<?php echo !empty($reviewed_at) ? $reviewed_at : '';  ?>" id="is_reviewed" <?php if($is_reviewed>0){echo "checked='checked'";}?>>
                                                                </div>
                                                                <input type="hidden" name="reviewed_by" value="<?php echo $u_id; ?>">
                                                                <div class="content">
                                                                    <p id="supervisor_wrap" style="<?php echo $is_reviewed>0 ? 'display:block;': 'display: none;' ; ?>">
                                                                    <?php echo  substr($logged_user[0]['first_name'],0,1).substr($logged_user[0]['last_name'],0,1);
                                                             ?> <span id="current_time"><?php echo $reviewed_at!="0000-00-00 00:00:00" ? date('m/d/Y h:i:s A',strtotime($reviewed_at)) : ''; ?></span>
                                                                    </p>    
                                                                </div>
                                                            </div>
                                                        </div>
