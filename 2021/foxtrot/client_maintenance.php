...
if((isset($_POST['submit'])&& $_POST['submit']=='Save') 
        ...
        || (isset($_POST['submit'])&& $_POST['submit']=='Next') )
    {
        ...
        $reviewed_at = isset($_POST['is_reviewed'])?$instance->re_db_input($_POST['is_reviewed']):'';
        $reviewed_by = isset($_POST['reviewed_by'])?$instance->re_db_input($_POST['reviewed_by']):'';
    ...
    else if(isset($_GET['delete_action'])&&$_GET['delete_action']=='delete_attach'&&isset($_GET['attach_id'])&&$_GET['attach_id']>0)
    {
       ...
       $return = $instance->search($search_text);
        ...
        $is_reviewed = isset($return['is_reviewed'])?$instance->re_db_output($return['is_reviewed']):0;
        $reviewed_at = isset($return['reviewed_at'])?$instance->re_db_output($return['reviewed_at']):'';
        $reviewed_by = isset($return['reviewed_by'])?$instance->re_db_output($return['reviewed_by']):'';
