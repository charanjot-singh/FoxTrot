<?php
	class client_maintenance extends db{
		...
		public function insert_update($data) {
			...
			$for_import = isset($data['for_import'])?$this->re_db_input($data['for_import']):'';
					...
					return $this->errors;
						...
						$q = "INSERT INTO `".$this->table."`"
								." SET `first_name`='".$fname."'"
										...
										.",`split_rate_to`='".$split_rate_to."'"
										.",`split_rate_from`='".$split_rate_from."'"
										.",`split_rate_category`='".$split_rate_category."'"
										.",`file_id`=$file_id"
						...
						if($res){
							if ($for_import == 'reprocess_add_client_on_the_fly'){
								$_SESSION[$for_import]['insert_update_master'] = $client_id;
							}
        ...
        public function insert_update_account($data){
            ...
            if($id==0){
                    ...
                    {
                    	if(empty($val_acc)) continue;

                    	if(empty($sponsor[$key_acc])) continue;
